...
public class BorrowingService {
    ...
    public Borrowing borrowBook(Integer userId, Integer bookId, int borrowDays) throws SQLException {
        ...
        Borrowing borrowing = Borrowing.builder()
                ...
                .dueDate(dueDate)
                .status("borrowed")
                .notes("Dipinjam via BorrowingService - " + userId)
                .build();
    ...
    public double calculateFine(Integer borrowingId) throws SQLException {
        ...
        // Jika buku sudah dikembalikan, return existing fine amount
        ...
        long overdueDays =
                (System.currentTimeMillis() - borrow.getDueDate().getTime()) / (1000 * 60 * 60 * 24);
     ...
     * @param userId ID user
     * @return List of semua borrowing records
     * @throws SQLException jika operasi database gagal
     */
    ...
    /**
     * Update status borrowing menjadi overdue jika melewati due-date
    ...
    public void updateOverdueStatus() throws SQLException {
        ...
        for (Borrowing overdue : overdueBorrowings) {
            if (!"overdue".equals(overdue.getStatus())) {
                borrowingDAO.updateStatus(overdue.getBorrowingId(), "overdue");

                // Calculate and update fine amount
                double fine = calculateFine(overdue.getBorrowingId());

                borrowingDAO.updateFineAmount(overdue.getBorrowingId(), fine);

                logger.info("Updated to overdue - Borrowing ID: " +
                        overdue.getBorrowingId() + ", Fine: " + fine);
            }
        }
